...
| 3 | 참여자는 이름만 이용해 자신의 가용 시간을 등록하거나 수정할 수 있어야 한다. |
...
| 참여자 식별 기준 | 현재 v1 프론트엔드는 참여자 `name`만 사용한다. 비밀번호 입력은 허용하지 않으며, 추후 확장 가능성만 남겨 둔다. |
...
| 항목 | 규칙 |
| --- | --- |
| 식별 방식 | `Authorization: Basic <base64(name:)>` |
| `username` 의미 | 참여자 `name` |
| PATCH 유저 ID 의미 | PATCH에서 유저 ID는 참여자 `name`이다. |
| 비밀번호 세그먼트 | 현재 v1 프론트엔드에서는 항상 비워 둔다. 추후 확장 여지는 남겨 둔다. |
| 빈 비밀번호 인코딩 | `name:` 형태를 base64로 인코딩 |
| 인증 헤더 실패 | 헤더 누락, `Basic` 스킴 아님, base64 파싱 실패, `name:` 형식 불일치는 `INVALID_AUTH_HEADER`로 거부한다. |
...
| 같은 `name`이 있음 | 같은 참여자로 보고 기존 RRULE을 수정한다. |
| 이름 변경 시도 | 지원하지 않는다. |
...
| 순서 | 동작 |
| --- | --- |
| 1 | Basic 인증 헤더를 파싱해 `name`을 얻는다. |
| 2 | 이름이 비었거나 허용 문자 규칙에 어긋나면 거부한다. |
| 3 | 이벤트와 기존 참여자를 읽는다. |
| 4 | 같은 이름의 기존 참여자가 있으면 같은 참여자 수정 경로로 본다. |
| 5 | RRULE을 UTC RFC 5545 규칙으로 검증한다. |
| 6 | 이름 기준으로 새 참여자를 만들거나 기존 참여자를 수정한다. |
| 7 | 참여자 값 객체를 저장한다. |
...
| 1 | 이름만 사용하는 첫 참여자 생성 |
| 2 | 같은 이름으로 기존 참여자 수정 |
| 3 | 잘못된 Authorization 헤더 거부 |
| 4 | 빈 이름 거부 |
| 5 | 허용되지 않은 문자가 포함된 이름 거부 |
| 6 | 이벤트 범위를 벗어난 RRULE 거부 |
| 7 | 30분 단위를 벗어난 RRULE 거부 |
| 8 | 지원하지 않는 RRULE 필드 거부 |
| 9 | 잘못된 JSON 거부 |
| 10 | 필수 필드 누락 거부 |
| 11 | 잘못된 datetime 형식 거부 |
...
| 프론트엔드 페이지 경로 | 생성 페이지는 `/`, 참여/조회 페이지는 `/?{eventId}` 규칙을 사용한다. |
| 현재 프론트엔드 페이지 상태 | 루트 페이지에서 생성 흐름을 시작하고, 동일 URL `/?{eventId}` 안에서 로컬 세션 상태로 참여/조회 모드를 구분한다. |
| 프론트엔드 로컬 저장소 키 | `ejmn.participant.{eventId}` |
| 프론트엔드 로컬 저장소 값 | `{ name }` JSON 객체 |
| 프론트엔드 새로고침 규칙 | `GET /api/events/{eventId}` 후 로컬 저장소의 `name`이 현재 `members`에 있으면 조회 모드, 없으면 참여 모드다. |
